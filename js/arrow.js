...
class Arrow {
    ...
    update(inputDir, dt60, braking, maxSpeed = 1, boostActive = false) {
        ...
        // During drift+brake, skip realign to preserve the drift
        ...
        // During handbrake, grip is greatly reduced so the drift angle changes slowly
        const diff = angleDiff(this.moveAngle, this.facing);
        let effectiveGrip = boostActive ? CFG.GRIP * 0.5 : CFG.GRIP;
        if (driftBraking) effectiveGrip *= 0.2;
        ...
        // No recovery while braking (except handbrake — that only reduces grip)
